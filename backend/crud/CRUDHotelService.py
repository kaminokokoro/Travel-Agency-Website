...
from backend.db.model import HotelServices, generate_uuid, Hotel, UserRating
...
class CRUDHotelService:
        ...
        try:
            with session_scope() as db:
                ...
                if hotel_service_db is None:
                    ...
                    hotel_service_db.service_id = hotel_service.service_id
                    db.commit()
                    db.refresh(hotel_service_db)
                    return hotel_service_db
        except SQLAlchemyError as e:
    ...
    def get_hotel_service_by_hotel_id(self, hotel_id: str, page_number: int = 1, page_size: int = 10) -> HotelServices:
        try:
            with session_scope() as db:
                hotel_service_db = db.query(HotelServices).filter(
                    and_(HotelServices.hotel_id == hotel_id)).limit(page_size).offset((page_number - 1) * page_size).all()
                return hotel_service_db
        except SQLAlchemyError as e:
            error = str(e.__dict__['orig'])
            print(error)
